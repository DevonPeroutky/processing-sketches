"""
FlowField with multiple particle managers. One for ambient background particles and one creating new particles based on the emotions 
being read from /tmp/EMOTIONAL_PIPE
"""
...
def setup():
    ...
    emotional_particle_manager = FlowParticleFactory(
        max_lines=max_lines_number,
        left_x=angle_grid.left_x,
        right_x=angle_grid.right_x,
        top_y=angle_grid.top_y,
        bottom_y=angle_grid.bottom_y)
    ambient_particle_manager = FlowParticleFactory(
        max_lines=max_lines_number * 3,
        left_x=angle_grid.left_x,
        right_x=angle_grid.right_x,
        top_y=angle_grid.top_y,
        bottom_y=angle_grid.bottom_y)
    background_particle_manager = FlowParticleFactory(
        max_lines=max_lines_number / 4,
        left_x=angle_grid.left_x,
        right_x=angle_grid.right_x,
        top_y=angle_grid.top_y,
        bottom_y=angle_grid.bottom_y)
...
def draw():
    ...
    # print("{}-{}-{}".format(total_particles_created,
    #                         len(emotional_particle_manager.particles.keys()),
    #                         frameRate))
...
def update_configuration_from_emotions(emotions):
    ...
    if emotional_particle_manager:
        for emotion in emotions:
            emotional_particle_manager.generate_particles_from_emotion_payload(
                emotion)
