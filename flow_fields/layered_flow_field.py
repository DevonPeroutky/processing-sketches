...
def setup():
    ...
    build_angle_grid(angle_grid, num_rows, num_cols, noise_step,
                     z_noise_offset)
...
def build_angle_grid(angle_grid, num_rows, num_cols, noise_step,
                     z_noise_offset):
    print("Building the next angle grid with {} & {}".format(
        noise_step, z_noise_offset))
    y_noise_offset = 0
...
def draw():
    ...
    if frameCount <= 5:
        ...
        # 1. Generate the particles for this layer
        particle_manager.spawn_new_reed_groups(reed_width=200,
                                               reed_quantity=2000,
                                               left_x=left_x,
                                               top_y=top_y,
